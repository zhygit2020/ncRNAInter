...
class graphbuilder:
    ...
    def buildgraph_internal(self):
        ...
        with open(self.prj_path/'data'/'trainval_data'/'RNA_intrinsic'/'fea_mi_normalized.npy', 'wb') as f:
            np.save(f, mirna_nfea.numpy())
        with open(self.prj_path/'data'/'trainval_data'/'RNA_intrinsic'/'fea_lnc_normalized.npy', 'wb') as f:
            np.save(f, lncrna_nfea.numpy())
    ...
    def buildgraph(self, graph, Label_data_pair, mirna_merge_dim, lncrna_merge_dim, mirna_node_info, lncrna_node_info, row, id2nodeidx):
        ...
        if not u_id.values in graph.ndata['id'].numpy():
            graph_test.add_nodes(1, {'id': torch.tensor(u_id.values, dtype=torch.int64), 'nfea': u_nfea})
        ...
        if not v_id.values in graph.ndata['id'].numpy():
            graph_test.add_nodes(1, {'id': torch.tensor(v_id.values, dtype=torch.int64), 'nfea': v_nfea})
        ...
        graph_test.add_edges(u_idx, v_idx, {'edgeweight': torch.zeros(1, dtype=torch.float32).unsqueeze(-1), 'pair_id': torch.tensor(pair_id.values, dtype=torch.float32)})

        return graph_test, u_name.values, v_name.values, Labels.values

    def buildgraph_feaimp(self, graph, Label_data_pair, mirna_merge_dim, lncrna_merge_dim, mirna_node_info, lncrna_node_info, row, id2nodeidx):
        graph_leaveonefea = copy.deepcopy(graph)
        u_name = Label_data_pair['mirna'].to_frame().iloc[[row]]
        u_id = Label_data_pair['mirna_id'].to_frame().iloc[[row]]
        v_name = Label_data_pair['lncrna'].to_frame().iloc[[row]]
        v_id = Label_data_pair['lncrna_id'].to_frame().iloc[[row]]
