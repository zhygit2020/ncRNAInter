...
class Trainer:
    def __init__(self, params):
        self.params = params
        print('feature_type =', self.params.feature_type)
        print('n_epochs =', self.params.n_epochs)
        print('hidden_dim =', self.params.hidden_dim)
        print('KFold_num =', self.params.KFold_num)
        print('batch_size =', self.params.batch_size)
        print('learning_rate =', self.params.lr)
        print('KFold_val_type =', self.params.KFold_val_type)
        print('CUDA =', self.params.gpu)
    ...
    def train(self, graph_t, trainidxs, model, optimizer, kth_log_file):
        ...
        with tqdm.tqdm(train_dataloader, file=kth_log_file) as tq: 
            for step, (input_nodes, pair_graph, blocks_MFGS) in enumerate(tq):           
                ...
                # print(logits_t, batch_labels)
                # loss = F.mse_loss(logits_t, batch_labels, reduction = 'sum')
                loss = F.binary_cross_entropy(logits_t, batch_labels, reduction = 'sum')
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                ...
                tq.set_postfix({'edgeprediction[0]': '%.03f' % logits_t[0], 'edgelabel[0]': '%.03f' % batch_labels[0], 'batch_loss': '%.03f' % loss.item()}, refresh=False) # , refresh=False
    ...
    def validate(self, graph_v, edgeidxs, model, kth_log_file):
        val_dataloader = dgl.dataloading.EdgeDataLoader(graph_v, edgeidxs, self.sampler,
                                                        exclude = 'reverse_id',
                                                        reverse_eids = torch.cat([torch.arange(self.num_edges_single) + self.num_edges_single, torch.arange(self.num_edges_single)]), # !!!check
                                                        batch_size = self.params.batch_size,
                                                        shuffle = True,
                                                        drop_last = False,
                                                        num_workers = 1,
        ...
        with tqdm.tqdm(val_dataloader, file=kth_log_file) as tq:
            for step, (input_nodes, pair_graph, blocks_MFGS) in enumerate(tq):
                ...
                val_loss = F.binary_cross_entropy(logits_v, batch_labels, reduction = 'sum')
                # save info of batches in one epoch
    ...
    def split_KFold(self, split_type):
        if split_type == 'pair':
            ...
            # print(X) # [    0     1     2 ... 27691 27692 27693]
    ...
    def run_KFold(self, k, train, val, assess):
        ...
        _auc_t, _mcc_t, _precision_t, _recall_t, _specificity_t, _f1_t = 0,0,0,0,0,0
        _fprs_v, _tprs_v, _thresholds_v = 0,0,0
        _tn_v, _fp_v, _fn_v, _tp_v = 0,0,0,0
        _auc_v, _mcc_v, _precision_v, _recall_v, _specificity_v, _f1_v = 0,0,0,0,0,0
        _model = None
        _optimizer = None
        ...
        for epoch in range(self.params.n_epochs):
            ...
            epoch_edge_labels_t = epoch_edge_labels_t.cpu() # .squeeze(-1).numpy()
            epoch_edge_logits_t = epoch_edge_logits_t.cpu() # .squeeze(-1).numpy()
            epoch_edge_idxs_t = epoch_edge_idxs_t.cpu() # .squeeze(-1).numpy()
            # evaluate train
            ...
            print ('pid: ',os.getpid(), file=kth_log_file)
            print ('memory used: %.4f GB' % (psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024) , file=kth_log_file)
            ...
            # move to cpu if in gpu
            epoch_edge_labels_v = epoch_edge_labels_v.cpu() # .squeeze(-1).numpy()
            epoch_edge_logits_v = epoch_edge_logits_v.cpu() # .squeeze(-1).numpy()
            epoch_edge_idxs_v = epoch_edge_idxs_v.cpu() # .squeeze(-1).numpy()
            # evaluate train
            ...
            print(
                f">>>>Epoch {epoch:04d}<<<<"
                f"Train acc {acc_t:.4f}, Train Loss {loss_t:.6f}, train_MCC {mcc_t:.4f}, train_precision {precision_t:.4f}, train_specificity {specificity_t:.4f}, train_recall {recall_t:.4f}, train_f1 {f1_t:.4f}, "
                f"Val acc {acc_v:.4f}, Val Loss {loss_v:.6f}, val_MCC {mcc_v:.4f}, val_precision {precision_v:.4f}, train_specificity {specificity_v:.4f}, val_recall {recall_v:.4f}, val_f1 {f1_v:.4f}, "
                f"Train TN {tn_t}, Train FP {fp_t}, Train FN {fn_t}, Train TP {tp_t}, Val TN {tn_v}, Val FP {fp_v}, Val FN {fn_v}, Val TP {tp_v}, "
                f"Train AUC {auc_t:.4f}, Val AUC {auc_v:.4f}", file=kth_log_file)
        ...
        print(
            f">Epoch {_epoch:04d}<"
            f"Train acc {_acc_t:.4f}, Train Loss {_loss_t:.6f}, train_MCC {_mcc_t:.4f}, train_precision {_precision_t:.4f}, train_specificity {_specificity_t:.4f}, train_recall {_recall_t:.4f}, train_f1 {_f1_t:.4f}, "
            f"Val acc {_acc_v:.4f}, Val Loss {_loss_v:.6f}, val_MCC {_mcc_v:.4f}, val_precision {_precision_v:.4f}, train_specificity {_specificity_v:.4f}, val_recall {_recall_v:.4f}, val_f1 {_f1_v:.4f}, "
            f"Train TN {_tn_t}, Train FP {_fp_t}, Train FN {_fn_t}, Train TP {_tp_t}, Val TN {_tn_v}, Val FP {_fp_v}, Val FN {_fn_v}, Val TP {_tp_v}, "
            f"Train AUC {_auc_t:.4f}, Val AUC {_auc_v:.4f}", file=kth_log_file)
    ...
    def fit(self): 
        ...
        assess = ['loss_epochs', 'acc_epochs', 'fprs', 'tprs', 'thresholds', 'tn', 'fp', 'fn', 'tp', 'loss', 'acc', 'auc', 'mcc', 'precision', 'recall', 'specificity', 'f1']
...
if __name__ == '__main__':
    ...
    parser.add_argument("--filetype", default='csv', type=str, choices=['csv', 'npy'], help='feature data file type, csv or npy') # check
    parser.add_argument("--lr", type=float, default=1e-3, help="learning rate")
    parser.add_argument("--weight_decay", type=float, default=5e-4, help="Weight for L2 loss") 
    parser.add_argument("--n_epochs", type=int, default=1536, help="number of training epochs")
    parser.add_argument("--hidden_dim", type=int, default=128, help="number of hidden GraphSAGE units")
    parser.add_argument("--n_layers", type=int, default=2, help="number of hidden NodeSAGE layers")
    parser.add_argument("--test_rate", type=float, default=0.1, help="test rate for test RNAs spliting")
    parser.add_argument("--task_type", default='trainval', type=str)
    parser.add_argument("--feature_type", default='sequence', type=str) # sequence physico_chemical structure all
    parser.add_argument("--feature_name", default=None, type=str) # not used
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--KFold_num", type=int, default=5, help="number of folds for K-Fold Validation, default 5")
    parser.add_argument("--predict_mode", type=str, default='test', help="predict task type, test or predict")
    parser.add_argument("--KFold_val_type", type=str, default='pair', choices=['pair', 'mirna', 'lncrna'], help="KFold dataset split type, pair, mirna or lncrna")
