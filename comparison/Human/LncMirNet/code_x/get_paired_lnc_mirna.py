...
def get_mirna_lncrna_seq(paired_path,mirna_path,lncrna_path):
    ...
    '''
    lnckeys = []
    for key in lncrna.keys():
        lnckeys.append(key.split('|',1)[0])
    lnckey2lnc = dict(zip(lnckeys,lncrna.keys()))
    ...
    for (lnc,mir) in lncrna_mirna_paired:
        ...
        if lnc in lnckeys and mir in list(mirna.keys()):
            lnc = lnckey2lnc[lnc]
...
lncrna_path = "./data/gencode.v38.lncRNA_transcripts.fa"
