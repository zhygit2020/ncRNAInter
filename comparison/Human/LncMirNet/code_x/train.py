...
# positive_pairs_path = "./_data/lnc_mir_pairs.txt"
# negative_pairs_path = "./_data/negative_pairs.txt"
...
for train_index, test_index in kf.split(list(range(len(all_mats_np)))):
    ...
    model = get_model()
    adma = optimizers.adam(lr=0.0005)
    # adma = optimizers.adam(lr=0.000005)
    model.compile(optimizer=adma,
                  loss=losses.binary_crossentropy,
                  metrics=['accuracy'])
    # performance = CLA_EarlyStoppingAndPerformance(([train_data], train_label), ([test_data], test_label), 
    #                                                   patience = 50, 
