...
test_dir=prj_path/'Datasets'/'Training-validation_dataset'
...
import pandas as pd
new = pd.read_csv(test_dir/'trainval_pairs_run.csv')
miRna_tra = []
temp = []
lncRna_tra = []
y_tra = []
# for sequence in new:
for index,sequence in new.iterrows():
    # sequence = sequence.strip()
    # temp.append(sequence.split(',')[2])
    # lncRna_tra.append(sequence.split(',')[3])
    # y_tra.append(int(sequence.split(',')[6]))
    lncRna_tra.append(sequence.lncrna_seq)
    miRna_tra.append(sequence.mirna_seq.replace('U','T'))
    y_tra.append(int(sequence.Label))
# for c in temp:
...
print(X_mi_tra.shape)
print(X_lnc_tra.shape)
