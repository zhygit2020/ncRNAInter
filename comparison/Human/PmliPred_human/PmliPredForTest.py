...
def CNNBiGRU(traindata, trainlabel, testdata, testlabel, TotalSequenceLength, iteration):
    ...
    import h5py
    file=h5py.File(proj_path / 'trained_model' / 'CNNBiGRU' / f'epochsize_{GRUepochsize}'/f'CNNBiGRU_{iteration}thFold.h5')
    # import tensorflow as tf
    # print(file.attrs.get('keras_version'))
    # import sys
    # sys.exit()
...
def RF(trainfeature, trainlabel, testfeature, iteration):
    ...
    RFStruct = joblib.load(proj_path / 'trained_model' / 'RF' / f'CNNBiGRU_epochsize_{GRUepochsize}'/f'RF_{iteration}thFold.pkl')    
...
for iteration in range(K):
    ...
    dl_TP, dl_FP, dl_TN, dl_FN, dl_TPR, dl_TNR, dl_PPV, dl_NPV, dl_FNR, dl_FPR, dl_FDR, dl_FOR, dl_ACC, dl_F1, dl_MCC, dl_BM, dl_MK, dl_fprs, dl_tprs, dl_thresholds, dl_auc, dl_pres, dl_recs, dl_thresholds_prc, dl_auprc = comparisondeeplearning(testlabeldl, resultslabel_dl)
    ...
    ml_TP, ml_FP, ml_TN, ml_FN, ml_TPR, ml_TNR, ml_PPV, ml_NPV, ml_FNR, ml_FPR, ml_FDR, ml_FOR, ml_ACC, ml_F1, ml_MCC, ml_BM, ml_MK, ml_fprs, ml_tprs, ml_thresholds, ml_auc, ml_pres, ml_recs, ml_thresholds_prc, ml_auprc = comparisondeeplearning(testlabeldl, RFscore_ml)
    ...
    TP, FP, TN, FN, TPR, TNR, PPV, NPV, FNR, FPR, FDR, FOR, ACC, F1, MCC, BM, MK, fprs, tprs, thresholds, auc, pres, recs, thresholds_prc, auprc = comparisondeeplearning(testlabeldl, resultslabel_fz)
    ...
    print('AUC:', dl_auc, 'AUPRC:', dl_auprc, 'ACC:', dl_ACC, 'F1:', dl_F1, 'MCC:', dl_MCC, 'BM:', dl_BM, 'MK:', dl_MK)
    print('-------------RF_Result--------------')
