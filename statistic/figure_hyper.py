...
ax1.set_xticks(np.array(range(len(lr)))/key)
ax1.set_xticklabels(lr)
# ax1.set_xticks(np.array(range(len(lr)))/key, lr)
ax1.set_yticks(np.array(range(len(hidden)))/key)
ax1.set_yticklabels(hidden)
...
ax2.set_xticks(np.arange(scalerlimmin,scalerlimmax+0.1,0.1))
ax2.set_xticklabels([0.78,0.82,0.86,0.90,0.94])
