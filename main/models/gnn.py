...
class EdgePredictor(nn.Module):
    def __init__(self, num_classes, in_features):
        ...
        # self.actifun2 = nn.Softmax(dim=1) # F.cross_entropy() include logsofmax function and NLLoss function

    def apply_edges(self, edges):
        data = torch.cat([edges.src['x'], edges.dst['x']], 1)
        data = self.actifun1(self.W1(data))
        # logits = self.actifun2(self.W2(data))[:,1].unsqueeze(-1) # F.cross_entropy() include logsofmax function and NLLoss function
        logits = self.W2(data)
